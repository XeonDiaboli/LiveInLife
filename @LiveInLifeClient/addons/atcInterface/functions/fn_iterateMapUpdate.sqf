...
try
{
   ...
   private _currentVehicles = ((allMissionObjects "AllVehicles") select {
     		...
     		(((((getPosATL _vehicle) select 2) > _de)    OR  (_vehicle getVariable ["lilc_transponder_status", false])  )   && (({ (_vehicle inArea _x) } count lilc_atcInterface_airports) > 0) && (_vehicle isKindOf "Air")});
